...
function ireportDo(){
    ...
    var status =  $('input[name=status]:checked').val();
	    var url='${pageContext.request.contextPath}/Product/Admin/checkTxsq!iportYCTable.action?currentPage=${currentPage}&name='+name+'&insertTime='+insertTime+'&status='+status;
...
<div class="center">			
	...
	<div class="main" align="center">
		<h1 style="text-align: center;">异常提现用户记录</h1>
		...
		<td>备注</td>
		<td>提现方式</td>
			<td style="color: #f20000">当天提现次数</td>
		<td>操作</td>
		</tr>
		...
		<td><a class="a"  href="${pageContext.request.contextPath}/Product/Admin/userStat!loadUserInfo.action?username=${myel:jieMiUsername(list.userName)}">${myel:jieMiUsername(list.userName)}</a></td>
	    ...
	    <td>
			...
			<td style="color: #f20000">${list.txCount}</td>
		<td>
			<%-- <c:if test="${usersAdmin.userType eq '-1'}"><a class="a" href="javascript:requestTx('${list.id}','${list.txStatus}')">请求提现|</a></c:if> --%>
			<c:if test="${list.type eq '0'}"><a class="a" href="javascript:queryTx('${myel:jieMiUsername(list.users.username)}','${list.requestId}');">查看提现是否成功</a></c:if>
			<c:if test="${list.type eq '2'}"><a class="a" href="javascript:querylianlian('${list.requestId}','<fmt:formatDate value="${list.insertTime}" pattern="yyyyMMddHHmmss"/>');">查看提现是否成功</a></c:if>
...
$("#div_condition input[name='status']").click(function(){
	window.location.href="${pageContext.request.contextPath}/Product/Admin/checkTxsq!loadYCTxsq.action?name="+$("#name").val()+"&status="+$('input[name="status"]:checked').val()+"&insertTime="+$('#insertTime').val();
});
