...
<div class="center">			
	...
	<div class="main" align="center">
		<h1 style="text-align: center;">企业用户提现记录</h1>
		...
		<td><a class="a"  href="${pageContext.request.contextPath}/Product/Admin/userStat!loadUserInfo.action?username=${myel:jieMiUsername(list.usersCompanyId)}">${myel:jieMiUsername(list.usersCompanyId)}</a></td> 
	    ...
	    <td>
		    <c:choose>
		    	...
		    	<c:otherwise>宝付提现</c:otherwise>
		    </c:choose>
		</td>
		<td>
			<%-- <c:if test="${usersAdmin.userType eq '-1'}"><a class="a" href="javascript:requestTx('${list.id}','${list.txStatus}')">请求提现|</a></c:if> --%>
			<c:if test="${list.type eq '0'}"><a class="a" href="javascript:queryTx('${myel:jieMiUsername(list.usersCompanyId)}','${list.requestId}');">查看提现是否成功</a></c:if>
...
$("#div_condition input[name='status']").click(function(){
	window.location.href="${pageContext.request.contextPath}/Product/Admin/checkTxsq!loadTxsqCompany.action?name="+$("#name").val()+"&status="+$('input[name="status"]:checked').val()+"&insertTime="+$('#insertTime').val();
});
