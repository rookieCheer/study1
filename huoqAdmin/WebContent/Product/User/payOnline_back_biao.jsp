...
<link href="${pageContext.request.contextPath}/Product/css/pay_online.css" rel="stylesheet" type="text/css"/>
	...
	<script>
		...
		//是否使用红包
		var use_hb = true;
		var user_HB=0,user_HBtk,last_index;
		$(function(){
			...
			$(".hb").click(function(){
				...
				if(use_hb){
					if($(this).find(".on_red").css("display")=="block"){
						...
						user_HBtk=$(this).find(".red_big").attr("data-ticket");
						user_HB=coupon; 
					}
					last_index=$(this).index();
				}
				//var accountMoney = $("#txt_useAccountMoney").val();	//	接受输入的总额
				var copies = parseFloat("${inv.copies}");		//已选投资金额
				var leftMoney = parseFloat("${leftMoney * 0.01}");	//个人余额
				if(use_hb){
					if(leftMoney>=copies){
						...
						if(leftMoney+user_HB>copies){ 
							var num=parseFloat(copies-user_HB).toFixed(2);
							if(num>0) { 
								$("#txt_useAccountMoney").val(num);	
							}else { 
		...
		$("#payConfirm").click(function(){
			...
			$("#hidCoupon").val(user_HB*100);
			$("#hidCouponId").val(user_HBtk);
		...
		$("#txt_useAccountMoney").blur(function(){
			...
			if(user_HB > "${inv.copies}"){ 
				accountMoney=0;
				$(this).val(0);
			}else if (user_HB + accountMoney > "${inv.copies}") { 
				accountMoney="${inv.copies}" - user_HB;
				$(this).val(parseFloat(accountMoney));
			}
			var coupon = 0;
			if(!use_hb){
			...
			//自己支付的金额+投资券;
			var totalMoney = parseFloat(accountMoney)+parseFloat(coupon)+user_HB;
			//alert(totalMoney);
			if(totalMoney<copies){
				//显示在线支付
				var need_pay=(parseFloat(copies)-parseFloat(totalMoney)).toFixed(2);
				if(need_pay>0){
					$("#span_needPay").text(need_pay);
					$("#div_other_pay").attr("style","display:block;");
				}		
			}else{
				$("#span_needPay").text(0);
