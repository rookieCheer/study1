...
public class TxRequestThread implements Runnable {
        ...
        synchronized (LockHolder.getLock(txRecordId)) {
            try {
                ...
                if ("0".equals(txRecord.getStatus()) && "0".equals(txRecord.getTxStatus())) {
                    ...
                    log.info("-------------------------提现ID: " + txRecord.getId() + "------提现订单: " + orderId + "	用户ID: " + usersId);
                    ...
                    // 商户私钥
                    String pfxpath = path.substring(0, path.indexOf("WEB-INF")) + "CER/" + resb.getString("daifusy.name");
                    // 获取公钥
                    String cerpath = path.substring(0, path.indexOf("WEB-INF")) + "CER/" + resb.getString("daifugy.name");
                    // 请求宝付地址
                    ...
                    if (txOk) {
                        ...
                        smsNoticeBean.sendSMSInThreadPool(shortUrl, DESEncrypt.jieMiUsername(users.getUsername()), sendMessage,
                                                          new Object[] { QwyUtil.calcNumber(txRecord.getMoney(), 100, "/", 2) });

                    } else {
                        // 提现失败;修改TxRecord的记录状态;改为2;
                        boolean isOk = userRechargeBean.baoFuTixianFail(usersId, txRecord.getMoney());
                        if (isOk) returnMsg += ";提现金额已返还;";
                        else returnMsg += ";提现金额返还失败,请联系客服;";
                        txRecord.setStatus("2");
    ...
    public String checkUsersMoney(long usersId, String txRecordId) throws Exception {
        ...
        if (users != null) {
            ...
            if (info != null) {
                Double dfreeze = info.getFreezeMoney();
                if (dfreeze != null) {
                    freezeMoney = dfreeze.doubleValue();
                }
            }
        }
        // 校验合法收入,支出;计算可提现余额;
        ...
        dao.saveOrUpdate(txRecord);
        /*
         * boolean isGetEarnings = isAlreadyGetEarnings(usersId); if(!isGetEarnings){ //3天内没有收益,请人工审核
         * txRecord.setCheckTime(new Date()); String note = "3天内没有收益,请人工审核";
         * if(QwyUtil.isNullAndEmpty(txRecord.getNote())||!txRecord.getNote().contains(note)){
         * txRecord.setNote(note+txRecord.getNote());//失败原因; } dao.saveOrUpdate(txRecord); continue; } boolean
         * isRecharge = isRechargeByDate(txRecord.getUsersId(), "2017-02-22 09:00:00", "2017-02-23 12:00:00");
         */

        // 针对22和23号异常充值用户作过度处理,后期可注释;
        /*
         * if(isRecharge){ //22号和23号有充值,请人工审核 txRecord.setCheckTime(new Date()); String note = "22号和23号有充值,请人工审核";
         * if(QwyUtil.isNullAndEmpty(txRecord.getNote())||!txRecord.getNote().contains(note)){
         * txRecord.setNote(note+txRecord.getNote());//失败原因; } dao.saveOrUpdate(txRecord); continue; }
         */
    ...
    /**
     * 根据用户id获取用户的产品收益+红包返现; <br>
     * '状态 0未支付,1已冻结,2已支付,3已删除',
     * 
     * @param usersId
     * @return 收益金额(分)
     * @throws Exception 红包直接返现到账号余额 不走利息表 涉及到的表：fundRecord coupon 修改：wxl 2017年3月1日11:21:07
     */
