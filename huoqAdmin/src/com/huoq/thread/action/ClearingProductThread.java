 ...
 */
@Service
public class ClearingProductThread implements Runnable {
	private Logger log = Logger.getLogger(ClearingProductThread.class);
	...
	public void run() {
		try {
			...
			String[] investTypes = { "0", "1", "2", "3" };// 投资类别 0:车无忧
															// 1:贸易通;2:牛市通;3:房盈宝
			...
			for (int i = 0; i < investTypes.length; i++) {
				...
				for (;;) {
					...
					pageUtil = bean.getProductByPageUtil(pageUtil, new String[] { investTypes[i] }, new String[] { "2" }, new String[] { "0" }, new Date());
